...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		# Applies rotation matrix for body frame -> vehicle frame
		newPoints = mm.multiply(newPoints, (Rotations.euler2DCM(yaw, pitch, roll)))

		# Applies the displacement of the x, y, z by manually adding them to the proper points
		displacement = [x, y, z]
		for i in range(len(newPoints)):
			for j in range(len(newPoints[i])):
				newPoints[i][j] = newPoints[i][j] + displacement[j]

		# Applies the NED -> ENU conversion
